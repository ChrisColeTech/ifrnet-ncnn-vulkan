...
{
    fprintf(stderr, "Usage: ifrnet-ncnn-vulkan -0 infile -1 infile1 -o outfile [options]...\n");
    fprintf(stderr, "       ifrnet-ncnn-vulkan -i indir -o outdir [options]...\n\n");
    ...
    fprintf(stderr, "  -m model-path        ifrnet model path (default=IFRNet_Vimeo90K)\n");
...
void* proc(void* args)
{
    ...
    const IFRNet* ifrnet = ptp->ifrnet;
    ...
    for (;;)
    {
        ...
        ifrnet->process(v.in0image, v.in1image, v.timestep, v.outimage);
...
#endif
{
    ...
    int numframe = 0;
    float timestep = 0.5f;
    path_t model = PATHSTR("IFRNet_Vimeo90K");
    ...
    bool ifrnet_gopro = false;

    if (model.find(PATHSTR("IFRNet")) != path_t::npos && model.find(PATHSTR("Vimeo90K")) != path_t::npos)
    {
        // fine
    }
    else if (model.find(PATHSTR("IFRNet")) != path_t::npos && model.find(PATHSTR("GoPro")) != path_t::npos)
    {
        // fine
        ifrnet_gopro = true;
    }
    ...
    if (!ifrnet_gopro && (numframe != 0 || timestep != 0.5))
    {
        fprintf(stderr, "only GoPro model support custom numframe and timestep\n");
        return -1;
    }
...
//                 fprintf(stderr, "%d %f %d\n", i, fx, sx);
    ...
    {
        std::vector<IFRNet*> ifrnet(use_gpu_count);

        for (int i=0; i<use_gpu_count; i++)
        {
            int num_threads = gpuid[i] == -1 ? jobs_proc[i] : 1;

            ifrnet[i] = new IFRNet(gpuid[i], tta_mode, num_threads);

            ifrnet[i]->load(modeldir);
        }
